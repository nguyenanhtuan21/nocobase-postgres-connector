...
export class StandaloneDatabaseService extends SimpleService {
  ...
  async disconnect(client: DatabaseClient, type: string): Promise<void> {
    try {
      if (client && typeof client.end === 'function') {
        await client.end();
        this.logger.info(`${type} connection closed`);
      }
    } catch (error: any) {
      this.logger.error(`Error closing ${type} connection`, { error: error.message });
    }
  }
  ...
  validateConfig(config: DatabaseConfig): void {
    const required: (keyof DatabaseConfig)[] = ['type', 'host', 'port', 'database', 'username', 'password'];
